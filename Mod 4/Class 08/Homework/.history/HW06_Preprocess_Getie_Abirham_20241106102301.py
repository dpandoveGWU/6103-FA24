...
assignment_df["hrs1"] = df.apply(clean_hours_worked, axis=1)
assignment_df["childs"] = df.apply(clean_children, axis=1)
assignment_df["income"] = df.apply(clean_income, axis=1)
assignment_df["ballot"] = df.apply(clean_ballot, axis=1)
...
# 1. Box plot for hours worked last week by marital status
plt.figure(figsize=(10, 6))
sns.boxplot(data=assignment_df, x="marital", y="hrs1")
...
# %%
# 2. Violin plot for income vs happiness
plt.figure(figsize=(10, 6))
sns.violinplot(data=assignment_df, x="happy", y="income")
...
sns.scatterplot(
    ...
    x="childs", 
    y="happy", 
    hue="marital",  # Choose marital status as hue
    alpha=0.5
)
...
sns.scatterplot(
    data=assignment_df, 
    x="childs" + np.random.normal(0, 0.3, assignment_df.shape[0]),  # Jitter for x
    y="happy" + np.random.normal(0, 0.1, assignment_df.shape[0]),  # Jitter for y
    hue="marital",  
    alpha=0.6
)
plt.title("Happiness vs Number of Children with Jitter")
