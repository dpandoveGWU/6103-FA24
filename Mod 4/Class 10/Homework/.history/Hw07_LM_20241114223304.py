...
titanic_data = titanic.drop(columns=['id'], errors='ignore')

# Proceed with mapping and dummy variable creation
titanic['sex'] = titanic_data['sex'].map({'male': 1, 'female': 0})
titanic_data = pd.get_dummies(titanic_data, columns=['pclass'], drop_first=True)

# Fill missing values in 'age' column
titanic_data['age'].fillna(titanic_data['age'].median(), inplace=True)

# Select features and target variable as before
X = titanic_data[['sex', 'age', 'sibsp', 'parch', 'pclass_2', 'pclass_3']]
y = titanic_data['survived']

# Add constant to predictors
X = sm.add_constant(X)

# Proceed with the rest of the code as before to fit the model
...
# Map 'sex' to numeric and create dummy variables for 'pclass'
titanic['sex'] = titanic['sex'].map({'male': 1, 'female': 0})
titanic_data = pd.get_dummies(titanic, columns=['pclass'], drop_first=True)

# Fill missing values in 'age' column
titanic['age'].fillna(titanic['age'].median(), inplace=True)

# Select features and target variable
X = titanic[['sex', 'age', 'sibsp', 'parch', 'pclass_2', 'pclass_3']]
y = titanic['survived']

# Add constant to predictors
X = sm.add_constant(X)

# Replace any remaining inf values and drop rows with NaNs
X = X.replace([np.inf, -np.inf], np.nan).dropna()
y = y[X.index]  # Align y with X's index after dropping rows
...
# %%
# Ensure both X and y are not empty and have compatible shapes
if X.shape[0] == 0 or y.shape[0] == 0:
    raise ValueError("X or y has no data after removing NaNs and inf values.")

# Convert to NumPy arrays for compatibility with statsmodels
