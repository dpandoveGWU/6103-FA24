...
print("Data types in X before conversion:\n", X.dtypes)

# Convert all columns in X to numeric (coerce any non-numeric values to NaN)
X = X.apply(pd.to_numeric, errors='coerce')

# Check for any remaining NaN values in X or y and drop rows with NaN
X = X.dropna()
y = y[X.index]  # Ensure y matches X's index after dropping rows

# Convert X and y to NumPy arrays to avoid dtype issues in statsmodels
X = np.asarray(X)
y = np.asarray(y)
