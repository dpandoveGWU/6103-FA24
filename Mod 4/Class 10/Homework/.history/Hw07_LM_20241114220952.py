...
titanic_data['sex'] = titanic['sex'].map({'male': 1, 'female': 0})
...
# Check and remove rows with NaN or infinite values in X and y
X = X.replace([np.inf, -np.inf], np.nan).dropna()
y = y[X.index]  # Align y with the cleaned X

# Confirm all data is now valid
print("Remaining NaNs or Infs in X:", X.isnull().sum().sum())
print("Remaining NaNs or Infs in y:", pd.isnull(y).sum())
